+++
title = "[Koikatu] コイカツ！ 個人插件更新記錄"
description = "[Koikatu] コイカツ！ 個人插件更新記錄"
date = 2022-12-04T12:36:00.004Z
updated = 2023-09-30T18:12:35.830Z
draft = false
aliases = ["/2020/05/koikatu-update-log.html"]

[taxonomies]
tags = ["Koikatu"]

[extra]
banner = "https://img.maki0419.com/blog/preview/banner.png"
+++
↑↑Last Update Time

  
[![](https://img.maki0419.com/blog/preview/banner.png)](https://img.maki0419.com/blog/preview/banner.png)

[Github Commit History](https://github.com/jim60105/KK/commits/KKSunshine)

  
此頁只做Log記錄用，請到下方鏈結尋找下載和其它資訊。  
このページはロギング専用です。  
ダウンロードやその他の詳細については、このリンクにアクセスしてください。  
This page is only for logging, please go to this link for download and other details.  
[\>>> コイカツ！個人插件介紹匯整 <<<](/2020/05/personal-koikatu-plugin.html)

  
2023/10/01

###  Studio Chara Only Load Body

PV.v23.10.01.0

RV.v1.4.4

* Add compatibility with AdditionalFKNodes  
Currently there is no AdditionalFKNodes for KKS.  
This update is to keep it consistent with the KK version.

2023/10/01

### **_Koikatu_** Studio Chara Only Load Body

PV.v23.10.01.0

RV.v1.3.11

* Add compatibility with AdditionalFKNodes

---

2022/12/04

### Coordinate Load Option

PV.v22.12.04.3

RV.v1.4.7

* Fixed the issue where the ResolveInfo of unreplaced clothing/accessories would change from LocalSlot to Slot.
* Fixed the mistake in the calculation of the maximum number of MoreAccessories.
* MaterialEditor Support: Add Copy Material feature support.
* Code refactor

---

2022/11/04

### Studio Save Workspace Order Fix

PV.v22.11.04.0

RV.v1.1.1

* [Prevent crashes breaking studio save, for insurance](https://github.com/jim60105/KK/commit/8d11d1fee942d8fe20cb272e5606363a08e4054a) by ManlyMarco

2022/11/04

### **_Koikatu_**Studio Save Workspace Order Fix

PV.v22.11.04.0

RV.v1.0.2

* [Prevent crashes breaking studio save, for insurance](https://github.com/jim60105/KK/commit/21b10f820364544661d8cd90a1bb1923cc5f30c6) by ManlyMarco

2022/11/02

### **_Koikatu_**Studio Chika Replacer

PV.v22.11.02.0

RV.v1.1.2

* [Add non-darkness KK support](https://github.com/jim60105/KK/pull/18/commits/895a7f4403ac5b9322352664637d7f096dd739c1) by ManlyMarco

---

2022/06/30

### Coordinate Load Option

PV.v22.06.30.0

RV.v1.4.3

* When boundAcc, adjust UI display to avoid misunderstanding.

2022/06/30

### **_Koikatu_** Studio Chara Only Load Body

PV.v22.06.27.0

RV.v1.3.10

* [Fix crashing with MoreAccessories 2.x](https://github.com/jim60105/KK/commit/da5156c53ac75cbe3fee010ec2ffa8729fc9f2dc) by ManlyMarco

---

2022/05/08

### **_Koikatu_** Coordinate Load Option

PV.v22.05.08.0

RV.v1.1.9

* [Add MoreAccessories 2.x support](https://github.com/jim60105/KK/commit/9faa5584743423ccd8ab5569cb64fe7d55e22167) by ManlyMarco

---

2021/12/25

### **_Koikatu_** Coordinate Load Option

PV.v21.12.25.0

RV.v1.1.8.1

* 補上新機能在主動ClearAcc時，未正確清空髮飾品

---

2021/12/24

### Coordinate Load Option

PV.v21.12.23.0

RV.v1.4.2

1. 新增「在特定的插件資料存在時，關閉飾品選項功能」
2. Fix changes on ABMX v4.4.4
3. 移除MoreAcc相關的RemoveEmptyFromBackToFront功能
4. 兼容KKS\_MoreOutfits

2021/12/24

### **_Koikatu_** Coordinate Load Option

PV.v21.12.23.0

RV.v1.1.8

1. 新增「在特定的插件資料存在時，關閉飾品選項功能」
2. Fix NullRefException with HairAccCus Supports
3. Fix changes on ABMX v4.4.4 (Also change the dependent version to v4.4.4)

2021/12/24

### Save Load Compression

PV.v21.12.23.0

RV.v1.6.0

1. Chara、Coordinate、Scene的分別選項
2. Change default settings

2021/12/24

### Studio Chara Only Load Body

PV.v21.12.23.0

RV.v1.4.3

1. 兼容KKS\_MoreOutfits

---

2021/10/30

### Studio Chara Light Linked To Camera

PV.v21.09.24.0

RV.v1.2.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/10/30

### Studio Simple Color On Girls

PV.v21.09.24.0

RV.v1.3.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/10/30

### Studio Auto Close Loading Scene Window

PV.v21.09.27.0

RV.v1.1.1

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/10/30

### Studio Chika Replacer

PV.v21.09.27.0

RV.v1.2.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/10/30

### Studio Transgender Loading

PV.v21.09.27.0

RV.v1.1.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/10/30

### Studio Save Workspace Order Fix

PV.v21.09.28.0

RV.v1.1.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/10/30

### Studio Reflect FK Fix

PV.v21.09.28.0

RV.v1.1.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/10/30

### Studio Dual Screen

PV.v21.10.04.0

RV.v1.2.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0
3. 現在在重新Enable時會自動SetLock(false)

2021/10/30

### FBI Open Up

PV.v21.10.11.0

RV.v1.2.1

1. 重新在原生環境(無插件環境)產生sample\_chara.png

2021/10/30

### Studio Text Plugin

PV.v21.10.23.0

RV.v1.2.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/10/30

### Studio Chara Only Load Body

PV.v21.10.24.0

RV.v1.4.2

1. 升級至Koikatsu Sunshine
2. 調整依賴MoreAcc至2.0
3. 升級至.NET Standard 2.0

2021/10/30

### Coordinate Load Option

PV.v21.10.29.0

RV.v1.3.1

1. 調整依賴MoreAcc至2.0
2. 移除COBOC支援
3. 修正以在Studio中運作

---

2021/08/31

### Coordinate Load Option

PV.v21.08.31.0

RV.v1.2.2

1. 修正和HairAccCus支援相關的Null Reference Exception

---

2021/08/30

### Coordinate Load Option

PV.v21.08.29.2

RV.v1.2.1

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/08/30

### PNG Capture Size Modifier

PV.v21.08.29.0

RV.v1.6.1

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/08/30

### Save Load Compression

PV.v21.08.28.0

RV.v1.5.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/08/30

### Coordinate Capture Pose Unlock

PV.v21.08.28.0

RV.v1.1.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/08/30

### FBI Open Up

PV.v21.08.29.0

RV.v1.2.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

2021/08/30

### Plugin List Tool

PV.v21.08.29.0

RV.v1.1.0

1. 升級至Koikatsu Sunshine
2. 升級至.NET Standard 2.0

---

2021/01/04

### Coordinate Load Option

PV.v21.01.04.0

RV.v1.1.6

1. 更新以配合MaterialEditor v2.5
2. 升級BepInEx依頼至v5.4.5

---

2020/12/29

### Save Load Compression Web

PV.v20.12.29.1

RV.v-.-.-

1. Init: <https://slcweb.maki0419.com/>

2020/12/29

### Save Load Compression

PV.v20.12.29.0

RV.v1.4.0

1. 新增unzip浮水印，解決「舊有壓縮存檔轉存成未壓縮時，圖像上留有zip浮水印」的狀況
2. 程式重構
3. 存檔和舊有的完全通用，格式沒改

---

2020/12/25

### Studio Menu Scrolling Text

PV.v20.12.25.0

RV.v1.2.1

修正
1. (中日文)字元長度計算問題

---

2020/12/02

### Coordinate Load Option

PV.v20.11.28.0

RV.v1.1.5

修正:
1. 在無COBOC或HairAcc的環境之錯誤  
(調整了幾個null check)

---

2020/11/16

### Coordinate Load Option

PV.v20.11.15.0

RV.v1.1.4

調整:
1. **程式重構**，這在插件支援部份改了很多，希望我沒有弄壞什麼
2. Extension log改為使用BepLogger
修正:
1. 在COBOC上的換衣邏輯錯誤，並補上換衣邏輯圖  
[![](https://img.maki0419.com/blog/COBOC_Change_Logic.png)](https://img.maki0419.com/blog/COBOC%5FChange%5FLogic.png)
2. 清空Acc會造成MaterialEditor資料丟失
3. Maker中，未展開UI(以原生方式讀取)時不會反應飾品、服裝選擇項
4. Maker中，當前選中之飾品為髮飾品，且讀入衣裝之該欄位非髮飾品時，在髮飾品比對會失敗
5. 補回讀取完成音
6. 讀取時飾品由後數的空欄會被省略，這造成無法用空欄服裝卡覆寫來做部份清除
7. 修復反選髮飾品功能
8. 對MaterialEditor Data的Texture Scale/Offset換衣錯誤
9. ABMX裙讀入時的變型錯誤

---

2020/10/15

### Coordinate Load Option

PV.v20.10.15.0

RV.v1.1.1

修正:
1. Maker中換衣時，HairAccData會覆寫至第一套(校內)並清空其它套服裝的問題
2. Maker中，若原本的HairAccData有啟用ColorMatch，會在換完Acc後把飾品原生顏色回寫為HairMatchColor
3. 清除飾品按鈕，最多會留下40個空格的問題
4. Maker中Deselect服裝時的error

---

2020/10/3

### Coordinate Load Option

PV.v20.10.02.0

RV.v1.1.0

Maker UI修正:
1. UI不再顯示於存檔窗格
2. 如果沒有全勾原生的讀取toggle時，換衣會不完整
3. 開啟UI時在讀取\\存檔窗格切換時，下方的toggle框之隱藏邏輯錯誤

新增功能:

1. UI位置記憶，可在ConfigurationManager中Reset

---

2020/09/10

### What are you saying?

PV.v20.09.10.1

RV.v1.0.0

First Release

2020/09/10

### Studio Menu Scrolling Text

PV.v20.09.10.0

RV.v1.2.0

增加速度調節功能

2020/09/10

### Save Load Compression

PV.v20.09.07.0

RV.v1.3.5

修改token使之能通過CharacterReplacer  
修改數處字串插值，用正確的方式做格式處理

---

2020/09/04

### Studio Menu Scrolling Text

PV.v20.09.05.0

RV.v1.1.0

First Release

2020/09/04

### Studio Transgender Loading

PV.v20.08.30.0

RV.v1.0.2

修正:
* 男轉女後，anim→狀態之乳首滑桿不會出現
* 男轉女後，女體特有的調整項不會做動
* 男轉女後，頭和頸小一圈；反之亦然

---

2020/08/22

### Chara Overlays Based On Coordinate

PV.v20.08.22.0

RV.v1.3.8

修正在Studio中使用IrisSide時身體的白色錯誤

2020/08/21

### Save Load Compression

PV.v20.08.21.0

RV.v1.3.4

修正壓縮完Compare時的UI文字Overflow問題

---

2020/08/06

### Coordinate Load Option

PV.v20.08.06.0

RV.v1.0.1

原有的Studio Coordinate Load Option，改為可在Maker中執行

2020/08/06

### ~~Studio Coordinate Load Option~~

PV.v--.--.--.-

RV.v-.-.-

淘汰，以Coordinate Load Option取而代之

2020/08/06

### Studio Simple Color On Girls

PV.v20.08.05.0

RV.v1.2.0

* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行
* 新增:在男角色也新增「重置單色化顏色」功能，選項在Config中分開

2020/08/06

### PNG Capture Size Modifier

PV.v20.08.06.0

RV.v1.5.5

* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行
* 修改Extension中有關圖像處理的部份
* 新增Maker選窗列數上限

2020/08/06

### Studio Dual Screen

PV.v20.08.05.0

RV.v1.1.7

* 移除BepInEx.Harmony依賴，升級依賴下限至BepInEx5.1
* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行
* 適配VMDPlay v0.2.4

2020/08/06

### Studio Transgender Loading

PV.v20.08.05.0

RV.v1.0.1

* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行

2020/08/06

### Studio Chara Only Load Body

PV.v20.08.05.0

RV.v1.3.9

* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行

2020/08/06

### Studio Reflect FK Fix

PV.v20.08.05.0

RV.v1.0.4

* 移除BepInEx.Harmony依賴，升級依賴下限至BepInEx5.1
* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行
* 修改Extension中有關圖像處理的部份

2020/08/06

### Studio Text Plugin

PV.v20.08.05.0

RV.v1.1.6

* 移除BepInEx.Harmony依賴，升級依賴下限至BepInEx5.1
* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行

2020/08/06

### Studio Auto Close Loading Scene Window

PV.v20.08.05.0

RV.v1.0.4

* 移除BepInEx.Harmony依賴，升級依賴下限至BepInEx5.1
* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行
* Code整理

2020/08/06

### Plugin List Tool

PV.v20.08.05.0

RV.v1.0.4

* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行
* 移除未使用的NuGet套件

2020/08/06

### FBI Open Up

PV.v20.08.05.0

RV.v1.1.3

* 移除BepInEx.Harmony依賴，升級依賴下限至BepInEx5.1
* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行

2020/08/06

### Chara Overlays Based On Coordinate

PV.v20.08.05.0

RV.v1.3.7

* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行

2020/08/06

### Studio Chika Replacer

PV.v20.08.05.0

RV.v1.1.1

* 移除BepInEx.Harmony依賴，升級依賴下限至BepInEx5.1

2020/08/06

### Studio Chara Light Linked To Camera

PV.v20.08.05.0

RV.v1.5.4

* 移除BepInEx.Harmony依賴，升級依賴下限至BepInEx5.1
* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行

2020/08/06

### Studio Save Workspace Order Fix

PV.v20.08.05.0

RV.v1.0.1

* 移除BepInEx.Harmony依賴，升級依賴下限至BepInEx5.1
* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行

2020/08/06

### Transparent Background

PV.v20.08.05.0

RV.v1.0.1

* 移除BepInEx.Harmony依賴，升級依賴下限至BepInEx5.1

2020/08/06

### Save Load Compression

PV.v20.08.05.0

RV.v1.3.3

* 移除BepInEx.Harmony依賴，升級依賴下限至BepInEx5.1
* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行
* 修改Extension中有關圖像處理的部份

2020/08/06

### Coordinate Capture Pose Unlock

PV.v20.08.05.0

RV.v1.0.1

* 移除BepInEx.Harmony依賴，升級依賴下限至BepInEx5.1
* 修正Extension中有關Field、Property、Method的方法，使之能在無IPAPatcher的環境執行

---

2020/07/29

### PNG Capture Size Modifier

PV.v20.07.28.0

RV.v1.5.4

修正  
* 升Bep5.1壞掉的東西
* H Scene中開啟服裝選擇清單時的error
* Maker中Chara Window的寬度偏差

2020/07/29

### Chara Overlays Based On Coordinate

PV.v20.07.27.0

RV.v1.3.6

修正  
* Maker中，讀取角色時會受到服裝視窗skin toggle影響的問題
* 在讀取「無CharaOverlay服裝」時IrisSide會初始化  
(設計: Maker在coordinate中無插件資料時，無論有否勾上Load Skin toggle都不會做清空)

2020/07/29

### Studio Transgender Loading

PV.v20.07.27.0

RV.v1.0.0

跨性別替換功能，取代全女插件StudioAllGirlsPlugin

2020/07/29

### ~~Studio All Girls Plugin~~

PV.v--.--.--.-

RV.v-.-.-

淘汰，以Studio Transgender Loading取而代之

2020/07/29

### Studio Simple Color On Girls

PV.v20.07.27.1

RV.v1.1.0

新增在讀入角色/讀入Scene時強制重置單色化顏色 的功能

2020/07/29

### Plugin List Tool

PV.v20.07.27.1

RV.v1.0.3

修改路徑為相對路徑，對應「第一次產生Default Config時的路徑」為暫用路徑造成的問題

2020/07/29

### Studio Chara Only Load Body

PV.v20.07.27.1

RV.v1.3.8

修正圖標，Code整理

2020/07/29

### Studio Coordinate Load Option

PV.v20.07.26.0

RV.v3.3.6

適配MaterialEditor2.0.7、MoreAcc 1.1.0  
KCOX和ABMX支援做了重寫  
修正髮FK Bone問題

---

2020/06/23

### Studio Simple Color On Girls

PV.v20.06.23.0

RV.v1.0.8

修正   
* 白雞雞問題
* 會有兩個o\_body\_a的問題

---

2020/06/09

### Save Load Compression

PV.v20.06.10.0

RV.v1.3.2

修正當資料夾中有錯誤存檔時，會無法顯示FileWindow內容的問題

2020/06/09

### PNG Capture Size Modifier

PV.v20.06.08.0

RV.v1.5.2

* 可自訂分辨率文字圖像
* 可自訂分辨率位置、大小
* 分辨率可單獨開啟

2020/06/09

### Coordinate Capture Pose Unlock

PV.v20.06.07.0

RV.v1.0.0

First Release

2020/06/09

### Save Load Compression

PV.v20.06.09.0

RV.v1.3.1

First Release

2020/06/07

### Studio Body Overwrite Script

PV.v20.05.30.0

RV.v1.0.0.1

另存編碼 Big5 → UTF8

---

2020/05/31

### Chara Overlays Based On Coordinate

PV.v20.05.30.0

RV.v1.3.5

修正Studio中存檔時會報錯，因為多加了一個不需要的KKAPI.Studio.SaveLoad.StudioSaveLoadApi.SceneSave Event