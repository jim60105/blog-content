...
[extra]
banner = "https://img.maki0419.com/blog/azure-blob-storage-206-partial-content/preview.png"
+++
[![](https://img.maki0419.com/blog/azure-blob-storage-206-partial-content/preview.png)](https://img.maki0419.com/blog/azure-blob-storage-206-partial-content/preview.png)
...
我正在開發一個網頁上的影片播放器，使用[ngx-videogular](https://github.com/videogular/ngx-videogular) 套件來播放儲存在 Azure Blob Storage 的 mp4 檔案。雖然影片可以成功**從頭**播放，但是卻無法從指定的時間開始播放，並且拉動時間軸時也無法進行跳轉。

我在使用 F12 開發人員工具檢查時，發現前端程式碼有向伺服器發出了 `Range: bytes` 的要求，而伺服器則回應了 200，並且從檔案的開頭開始傳輸。 我相信前端套件是沒有問題的，這個狀況應該是後端伺服器（在這裡是 Azure Blob Storage）未回應 Partial Content。

它無法理解我們「請從檔案的第n個bytes傳給我」的要求。

[![](https://img.maki0419.com/blog/azure-blob-storage-206-partial-content/before.png)](https://img.maki0419.com/blog/azure-blob-storage-206-partial-content/before.png)

正常的情況下，伺服器應該要回應 206 並從指定的 bytes 開始傳輸。

[![](https://img.maki0419.com/blog/azure-blob-storage-206-partial-content/after.png)](https://img.maki0419.com/blog/azure-blob-storage-206-partial-content/after.png)

## 發現問題

開什麼玩笑，堂堂 Azure 不支援 Partial Content?

我查了一下，這問題在兩年前有人問過: [Stack Overflow](https://stackoverflow.com/q/61672811/8706033) ，下方的三個解答~~罕見的~~都是正確的。  
...
影像資料在寫入完後，才能夠根據資料建立索引，所以它是寫在影像資料的後方。這個設計在本地讀取時沒什麼問題，播放器可以直接存取到整個檔案，但是在線上串流時就很糟糕了。播放器必須要下載完一整個檔案，取得最末尾的索引資訊，才有辦法搜尋時間軸。無法索引的檔案會從檔案的起始開始緩衝，直到當下選取的時間點為止。
...
具體的工具、指令可以參考我另一篇文章的最後尾附錄。關鍵字為 mp4 faststart。
...
iscn://likecoin-chain/DRhAnHi6Vd38TifJUX0HTnBBg9JywfCa\_JvFp1HrS-E